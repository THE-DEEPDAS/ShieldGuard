...
const addCourseTeacher = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { newCourse, loggedTeacher }, "new course created")
    );
});
...
const enrolledcourseSTD = asyncHandler(async (req, res) => {
  ...
  const Student = await course
    .find({ enrolledStudent: stdID })
...
const enrolledcourseTeacher = asyncHandler(async (req, res) => {
  ...
  const teacher = await course
    .find({ enrolledteacher: teacherID })
    .select("-enrolledStudent -liveClasses -enrolledteacher");
...
const addClass = asyncHandler(async (req, res) => {
  ...
  if (conflictClass.length > 0) {
    throw new ApiError(
      400,
      "You already have another class for similar timing."
    );
  }
